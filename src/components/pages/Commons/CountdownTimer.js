...
const Countdown = (props) => {
  ...
  var minutesEnd = Math.floor((remainingToEnd % (1000 * 60 * 60)) / (1000 * 60));
  ...
  return (
    <div>
        ...
        </Link>:`Starts: ${new Date(props.event.start_date).toUTCString()}`}
        {!isOpen && <p>time until start {`${days}d${hours}h${minutes}m${seconds}s`}</p>}
        {isOpen && <p>time until end {`${daysEnd}d${hoursEnd}h${minutesEnd}m${secondsEnd}s`}</p>}
